...
def init_db() -> None:
    ...
    with get_conn() as conn:
        ...
        # Tracks uploads (unique by filename)
        cur.execute("""
        ...
        """)
        # Orders partitioned by (partner_id, period)
        cur.execute("""
        CREATE TABLE IF NOT EXISTS orders(
            ...
            UNIQUE(partner_id, period, order_id)
        );
        """)
        # Helpful index for queries
        cur.execute("""
        CREATE INDEX IF NOT EXISTS idx_orders_partition_date
        ON orders(partner_id, period, order_date DESC);
        """)
        conn.commit()
